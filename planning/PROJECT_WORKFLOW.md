...
The complete exhibition is preserved by the annotated tag and archive branch `full-exhibition-v1`. Current work continues on `simplified-presentation`. The simplified public page has four visible chapters: hero, six-stop handscroll, Wang Wei portrait/context, and further-reading links. Technical explanation belongs in source code and the presentation-support document, not in the visitor-facing page. Do not restore the grammar map, technology cards, textual evidence gallery, mineral workshop, speech playback, or other long-form modules on this branch. Preserve implementation depth through the shared bilingual state model, continuous SVG handscroll, accessible controls, responsive layout, and one purposeful CSS state animation.
