...
这是围绕王维《山居秋暝》制作的双语数字手卷。简化版不再铺陈大量文化模块，只保留一条清楚的体验路径：**进入手卷 → 浏览六个诗句停驻点 → 切换三种解释 → 查看四项前端实现**。
...
页面下半部分用四张卡片概括全部核心技术：语义 HTML、CSS 布局、CSS 状态动画、原生 JavaScript 状态同步。它们既是网页的一部分，也是课堂汇报的提词结构。
...
页面末尾只保留四项必要信息：原诗底本、项目英译、原创 SVG 图像、GitHub Pages 部署方式。原创画卷是诗句的视觉转译，不是古画，也不声称复原王维见过的真实地点。
...
4. 滚动到四项技术卡，按 HTML → CSS → Motion → JavaScript 解释实现。
